...
const Layout = ({ children, activeTab, setActiveTab, isDarkMode, toggleTheme }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <aside className={`w-16 md:w-20 flex flex-col items-center py-8 border-r transition-colors duration-300 ${isDarkMode ? 'bg-[#1a1a1a] border-gray-800' : 'bg-white border-slate-200'}`}>
        ...
        <nav className="flex flex-col gap-8 flex-1">
          ...
          <button 
            ...
            className="p-2 rounded-xl text-gray-400 hover:text-[#c2c2f6] transition-all"
          >
            <Maximize2 size={24} />
          </button>
        </nav>
        ...
        <div className="flex flex-col gap-6 items-center">
          ...
          {user ? (
            <button 
              onClick={logout}
              className="p-2 rounded-full text-red-400 hover:bg-red-500/10 transition-all"
              title="Logout"
            >
              <LogOut size={20} />
            </button>
