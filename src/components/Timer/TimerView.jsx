...
const TimerView = () => {
    const { mode, timeLeft, isActive, startTimer, pauseTimer, resetTimer, skipSession, setMode, sessionsCompleted, dailyGoal } = useTimer();
    const { user } = useAuth();
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[80vh] space-y-8 animate-in fade-in duration-700">
                ...
                <div className="w-full max-w-md bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-gray-400">
                            Daily Goal: {sessionsCompleted}/{dailyGoal}
                        </span>
            ...
            <div className="relative group">
                <CircularProgress size={320} strokeWidth={8} percentage={progress * 100} color="#c2c2f6" />
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span className="text-7xl font-mono font-bold tracking-tighter tabular-nums drop-shadow-md">{formatTime(timeLeft)}</span>
                    <span className="text-gray-400 mt-2 font-medium uppercase tracking-widest text-xs opacity-60">{mode.replace("_", " ")}</span>
                </div>
            </div>
